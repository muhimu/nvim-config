...
-- Strip trailing whitespace on save (spaces/tabs at end of lines, not final newline)
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sf', function()
        ...
        local dirs = { cwd }
        if vim.uv.fs_stat(cwd .. '/local') then
          table.insert(dirs, cwd .. '/local')
        end
  ...
  { -- You can easily change to a different colorscheme.
    ...
    config = function()
      ...
      -- Load the generic `catppuccin` scheme (NOT a flavor-pinned one like
      -- `catppuccin-mocha`) so flavour='auto' can pick mocha/latte from
      -- vim.o.background. Ghostty reports its background via OSC 11, so when it
      -- flips Latte/Mocha with the OS, Neovim's `background` updates to match.
      vim.cmd.colorscheme 'catppuccin'

      -- Re-apply the colorscheme when the OS appearance flips mid-session, so a
      -- running Neovim switches light/dark without a restart.
